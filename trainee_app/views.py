...
class TraineeCreateView(View):
    ...
    def post(self, request):
        form = TraineeForm(request.POST, request.FILES)  # Add request.FILES to handle image uploads
        if form.is_valid():
            course_id = request.POST.get('course')
            course = Course.objects.get(id=course_id) if course_id else None
            Trainee.objects.create(
                ...
                phone=form.cleaned_data['phone'],
                course=course,
                image=form.cleaned_data['image']  # Save image
            )
            return redirect('trainee_list')
        courses = Course.objects.all()
        return render(request, self.template_name, {'form': form, 'courses': courses})
...
class TraineeUpdateView(View):
    ...
    def post(self, request, pk):
        trainee = get_object_or_404(Trainee, id=pk)
        form = Traineeformmodel(request.POST, request.FILES, instance=trainee)  # Add request.FILES
        if form.is_valid():
            form.save()
            return redirect('trainee_list')
        courses = Course.objects.all()
